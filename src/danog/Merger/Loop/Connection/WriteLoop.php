...
class WriteLoop extends ResumableSignalLoop
{
    ...
    public function loop(): \Generator
    {
        $socket = $this->socket;
        $id = $this->id;
        $stats = Stats::getInstance($this->id);

        $this->startedLoop();
        $API->logger->logger("Entered write loop in $id", Logger::ULTRA_VERBOSE);

        while (true) {
            if (empty($connection->pending_outgoing)) {
                if (yield $this->waitSignal($this->pause())) {
                    $API->logger->logger("Exiting write loop in $id");
                    $this->exitedLoop();
                    yield new Success(0);
    ...
    public function unencryptedWriteLoopAsync(): \Generator
    {
        ...
        while ($connection->pending_outgoing) {
            ...
            foreach ($connection->pending_outgoing as $k => $message) {
                ...
                $API->logger->logger("Sending {$message['_']} as unencrypted message to $id", \danog\MadelineProto\Logger::ULTRA_VERBOSE);
                ...
                $API->logger->logger("Sent {$message['_']} as unencrypted message to $id!", \danog\MadelineProto\Logger::ULTRA_VERBOSE);
    ...
    public function encryptedWriteLoopAsync(): \Generator
    {
        ...
        do {
            ...
            foreach ($connection->pending_outgoing as $k => $message) {
                ...
                $API->logger->logger("Sending {$message['_']} as encrypted message to $id", \danog\MadelineProto\Logger::ULTRA_VERBOSE);
            ...
            if (count($messages) > 1) {
                $API->logger->logger("Wrapping in msg_container as encrypted message for $id", \danog\MadelineProto\Logger::ULTRA_VERBOSE);
            ...
            $API->logger->logger("Sent encrypted payload to $id", \danog\MadelineProto\Logger::ULTRA_VERBOSE);
